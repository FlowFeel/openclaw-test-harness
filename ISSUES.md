...
- **Problem**: `patches/worker-pool.js` builds each worker via `new Worker(code, { eval: true })` where `code` is one giant `if (handler === 'json.stringify') … else if (handler === 'json.parse') …` string (worker body, ~lines 36–72), and the **same** dispatch is duplicated again in the inline fallback (~lines 130–148). Two hand-maintained copies of every handler; adding a handler means editing two places; the blob is untestable and diverges from the clean pure functions already in `ts/src/features/worker-pool/handlers.ts` (`registerBuiltinHandlers`).
- **Solution**: A single handler registry is the source of truth. The worker entry imports the registry and dispatches with `handlers.get(name)(input)` (3 lines). The inline fallback imports the **same** registry — zero duplication. The registry is derived from `handlers.ts`, not reimplemented. Handlers stay pure functions (input → output).
- **Acceptance**: (1) No handler logic appears twice; (2) adding a handler = one file + one `register()` call, no dispatch edit; (3) a test asserts the worker dispatch and the inline fallback return identical results for every built-in handler.
- **Status**: 📋 Planned
