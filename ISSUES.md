...
## #18: Fix oc-topic-worker-pool slot leak on hook failure / abort + dead-slot watchdog
- **Problem**: `oc-topic-worker-pool` leaked all 3 of its main-pool slots in production (`active=3/3`, `totalActive=0`), deadlocking all incoming Telegram topic agent runs for 6 hours. When the pool is saturated, queued runs awaiting `mainPool.acquire()` in `before_agent_run` hit OpenClaw's 15s modifying-hook timeout (`attempt-before-agent-run.ts:103`), causing OpenClaw to log `"before_agent_run hook failed; blocking request"` and block the turn. Because the run is blocked at the gate, OpenClaw never fires `agent_end`. Furthermore, the timed-out waiter remained in `createAsyncSemaphore.waiters[]`; when active runs finished, `release()` handed freed slots to already-dead waiters (`acquire(state)` incremented `active` back to 3), permanently abandoning slots to ghost runs. Additionally, `runId` resolution in `before_agent_run` checked only `event` (where `runId` is undefined in upstream OpenClaw) while `agent_end` checked `ctx.runId`, causing `runPoolMap.get(runId)` to return undefined and miss slot release entirely.
- **Solution**:
  1. **Cancellation & Timeout-Aware Semaphore**: Update `acquire({ timeoutMs, signal })` to evict timed-out and aborted waiters from the queue. When `release()` runs, purge dead waiters before slot reallocation so ghost waiters never consume slots.
  2. **Bounded Hook Queue Admission**: `before_agent_run` uses a bounded queue wait (default 10s < OpenClaw's 15s budget) and returns a clean `{ outcome: "block", reason: "Topic worker pool saturated (queue timeout)" }` on timeout rather than crashing the hook.
  3. **Guaranteed Cleanup on Error / Abort**: Robust `runId` resolution inspecting both `event` and `ctx`. If `acquire()` succeeds but `before_agent_run` throws or aborts, immediately release the slot in `catch/finally`.
  4. **Dead-Slot Watchdog & Self-Healing**: Pure lease tracking (`reapStaleLeases`, `reconcilePool`) with an autonomous periodic watchdog (or sweep check). If `pool.state.active > 0` but active live leases are 0, or if a lease exceeds `maxLeaseDurationMs` (default 5m), force-release the stale slots with alert logging to prevent permanent starvation. Apply identical protection symmetrically to `subPool`.
- **Acceptance**:
  1. ✅ Waiter timeout: under `max=1` with an active run, a second `acquire({ timeoutMs: 50 })` times out, evicts itself from `waiters`, and leaves `active=1`; when the active run releases, `active` drops to `0`.
  2. ✅ Ghost waiter eviction: released slots skip timed-out/aborted waiters and immediately return to available capacity.
  3. ✅ Hook failure release: if `before_agent_run` encounters an error post-acquire, the slot is immediately released.
  4. ✅ Production incident replay: 3 runs saturate pool, 3 runs timeout/abort at 15s; when the first 3 finish, `active` drops to 0, hook failures cease, and new runs acquire immediately without restart.
